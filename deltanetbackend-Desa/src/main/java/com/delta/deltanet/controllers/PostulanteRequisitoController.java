...
import com.delta.deltanet.models.service.PostulanteRequisitoService;

@CrossOrigin(origins= {"*"})
@RestController
@RequestMapping("/postulante-requisito")
public class PostulanteRequisitoController {

    @Autowired
    private PostulanteRequisitoService service;

    @GetMapping("/index")
    public ResponseEntity<?> getAll() {
        List<PostulanteRequisito> resultados = service.findAll();
    ...
    public ResponseEntity<?> findById(@RequestParam Long id) {
        PostulanteRequisito res = service.findById(id);
        if (res == null) {
            Map<String, Object> response = new HashMap<>();
            response.put("message", "No se encontró requisito de postulante con ID: " + id);
            return new ResponseEntity<>(response, HttpStatus.NOT_FOUND);
        }
        Map<String, Object> response = new HashMap<>();
        response.put("message", "Requisito de postulante encontrado con éxito");
    ...
    public ResponseEntity<?> create(@RequestParam String descripcion,
        ...
        PostulanteRequisito reg = new PostulanteRequisito();
        reg.setDescripcion(descripcionTrim);
        reg.setEstado(1);
        reg.setCreateUser(createUser);
        reg.setCreateDate(new Date());
        reg = service.save(reg);
        response.put("message", "Requisito de postulante creado con éxito");
    ...
    public ResponseEntity<?> update(@RequestParam Long id,
        ...
        PostulanteRequisito reg = service.findById(id);
        ...
        reg = service.save(reg);
        response.put("message", "Requisito de postulante se ha actualizado con éxito");
        response.put("data", reg);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<?> changeStatus(@RequestParam Long id,
        ...
        if (reg == null) {
            Map<String, Object> response = new HashMap<>();
            response.put("message", "No se encontró requisto de postulante con ID: " + id);
            return new ResponseEntity<>(response, HttpStatus.NOT_FOUND);
        }
        Map<String, Object> response = new HashMap<>();
        ...
        switch (estadoActual) {
            case 1:
                nuevoEstado = 0;
                mensaje = "Requisito de postulante desactivado con éxito";
                break;
            case 0:
                nuevoEstado = 1;
                ...
                mensaje = "Requisito de postulante activado con éxito";
                break;
            default:
                nuevoEstado = 0;
                mensaje = "Requisito de postulante desactivado con éxito";
