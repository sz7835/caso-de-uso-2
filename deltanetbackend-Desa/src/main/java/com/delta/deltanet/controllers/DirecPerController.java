...
import com.delta.deltanet.models.service.DireccionServiceImpl;
...
@CrossOrigin(origins = { "*" })
@RestController
@RequestMapping("/dirPer")
public class DirecPerController {
	...
	@Autowired
	private DireccionServiceImpl direccionService2;
	...
	@PostMapping("/save")
	public ResponseEntity<?> grabaDir(	@RequestParam("idPersona") Long idPersona,
		...
		try {
			...
			if (persona == null) {
				response.put("mensaje",
						"El ID Persona: ".concat(idPersona.toString().concat(" no existe en la base de datos")));
				return new ResponseEntity<>(response, HttpStatus.UNPROCESSABLE_ENTITY);
			}

			// Validar si la dirección ya existe para la persona
			List<Direccion> direccionesExistentes = direccionService.findByPersonaAndDireccion(idPersona, direccion);
	...
	@GetMapping("/index")
	public ResponseEntity<?> ListaDirecciones(	@RequestParam(name = "idPersona") Long idPer,
	...
	@PostMapping("/search")
	public ResponseEntity<List<lstDireccion1>> searchDirecciones(
			@RequestParam(value = "idPersona", required = false) Long idPersona,
			@RequestParam(value = "tipo", required = false) Long tipo,
			@RequestParam(value = "direccion", required = false) String direccion,
			@RequestParam(value = "tipoUbigeo", required = false) String tipoUbigeo,
			@RequestParam(value = "estado", required = false) Integer estado) {

		List<lstDireccion1> direcciones = direccionService2.search(idPersona, tipo, direccion, tipoUbigeo, estado);
	...
	public ResponseEntity<?> muestraDir(@RequestParam(name = "idDireccion") Long idDireccion) {
		...
		try {
			...
			if (direccion == null) {
				response.put("mensaje",
						"La direccion ID: ".concat(idDireccion.toString().concat(" no existe en la base de datos")));
				return new ResponseEntity<>(response, HttpStatus.UNPROCESSABLE_ENTITY);
			}
			List<Departamento> departamentos = departamentoService.findByAll();
			List<Object> provincias = provinciaService
					.lstPorDpto(direccion.getDistrito().getProvincia().getDepartamento().getId());
			...
			if(direccion.getPersona().getArea() != null) {
				direccion.getPersona().getArea().setPuestos(null);
				direccion.getPersona().getArea().getGerencia().setOrgAreas(null);
			}
			direccion.getPersona().setPuesto(null);
			response.put("direccion", direccion);

			return new ResponseEntity<>(response, HttpStatus.OK);
		} catch (Exception e) {
			response.put("mensaje", "Error al realizar la busqueda de direcciones");
			response.put("error", e.getMessage());
			return new ResponseEntity<>(response, HttpStatus.UNPROCESSABLE_ENTITY);
		}
	}
	...
	public ResponseEntity<?> UpdateDir(	@RequestParam(name = "idDireccion") Long idDire,
		...
		try {
			...
			if (registro == null) {
				response.put("mensaje",
						"La direccion ID: ".concat(idDire.toString().concat(" no existe en la base de datos")));
				return new ResponseEntity<>(response, HttpStatus.UNPROCESSABLE_ENTITY);
			}
			...
			if (idDist != null) {
				...
				if (distrito == null) {
					response.put("mensaje",
							"El distrito ID: ".concat(idDist.toString().concat(" no existe en la base de datos")));
					return new ResponseEntity<>(response, HttpStatus.UNPROCESSABLE_ENTITY);
				}
			...
			response.put("mensaje", "Error al realizar la actualizacion de direcciones");
			response.put("error", e.getMessage());
			return new ResponseEntity<>(response, HttpStatus.UNPROCESSABLE_ENTITY);
		}
	...
	public ResponseEntity<?> updEstado(@RequestParam(name = "id") Long id,
		...
		try {
			...
			if (direccion == null) {
				response.put("mensaje",
						"La direccion ID: ".concat(id.toString().concat(" no existe en la base de datos")));
				return new ResponseEntity<>(response, HttpStatus.UNPROCESSABLE_ENTITY);
			}
