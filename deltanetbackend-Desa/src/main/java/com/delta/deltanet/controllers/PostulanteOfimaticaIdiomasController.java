...
import com.delta.deltanet.models.service.IPostulanteOfimaticaIdiomasService;

@CrossOrigin(origins= {"*"})
@RestController
@RequestMapping("/postulante-ofimaticaidiomas")
public class PostulanteOfimaticaIdiomasController {

	@Autowired
    private IPostulanteOfimaticaIdiomasService service;

    @GetMapping("/index")
    public ResponseEntity<?> getAll() {
    	List<PostulanteOfimaticaIdiomas> resultados = service.findAll();
    ...
    public ResponseEntity<?> findById(@RequestParam Long id) {
    	PostulanteOfimaticaIdiomas res = service.findById(id);
    	if (res == null) {
    		Map<String, Object> response = new HashMap<>();
    		response.put("message", "No se encontró ofimática idioma de postulante con ID: " + id);
    		return new ResponseEntity<>(response, HttpStatus.NOT_FOUND);
    	}
    	Map<String, Object> response = new HashMap<>();
    	response.put("message", "Ofimática idioma de postulante encontrado con éxito");
    ...
    public ResponseEntity<?> create(@RequestParam String descripcion,
        ...
        PostulanteOfimaticaIdiomas reg = new PostulanteOfimaticaIdiomas();
        reg.setDescripcion(descripcionTrim);
        reg.setEstado(1);
        reg.setCreateUser(createUser);
        reg.setCreateDate(new Date());
        reg = service.save(reg);
        response.put("message", "Ofimática idioma de postulante creado con éxito");
    ...
    public ResponseEntity<?> update(@RequestParam Long id,
    	...
    	PostulanteOfimaticaIdiomas reg = service.findById(id);
        ...
        reg = service.save(reg);
        response.put("message", "Ofimática idioma de postulante se ha actualizado con éxito");
        response.put("data", reg);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<?> changeStatus(@RequestParam Long id,
        ...
        switch (estadoActual) {
            case 1:
                nuevoEstado = 0;
                mensaje = "Ofimática idioma de postulante desactivado con éxito";
                break;
            case 0:
                nuevoEstado = 1;
                ...
                mensaje = "Ofimática idioma de postulante activado con éxito";
                break;
            default:
                nuevoEstado = 0;
                mensaje = "Ofimática idioma de postulante desactivado con éxito";
